...
namespace FlightBooking.Core
{
    public class ScheduledFlight
    {
        ...
        private readonly List<IFlightBusinessRule> _flightBusinessRules;
        private readonly List<Plane> _availablePlanes;
        ...
        public string GetSummary()
        {
            ...
            foreach (var passenger in Passengers)
            {
                switch (passenger.Type)
                {
                    ...
                    case (PassengerType.Discounted):
                        {
                            profitFromFlight += FlightRoute.BasePrice / 2D;
                            break;
                        }
                    default:
            ...
            if(_flightBusinessRules.All(p => p.Pass(FlightRoute, Aircraft, Passengers)))
            {
            ...
            else
            {
                ...
                var availablePlanes = new Options().GetSuitableAlternatives(Passengers, _availablePlanes);
                if(availablePlanes.Any())
                {
                    result += _newLine;
                    result += "Other more suitable aircraft are:";
                    availablePlanes.ForEach(p => { result += _newLine; result += $"{p.Name} could handle this flight."; });
                }
            }
